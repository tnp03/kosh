...
function HomePage() {
  const { articles: allArticles, nodes: allNodes, sites: allSites } = Route.useLoaderData()
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        ...
        <p className="text-muted-foreground text-sm mb-6">
          Live overview of the knowledge base
        </p>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-card rounded-xl border p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  View full graph <ArrowUpRight className="h-3 w-3" aria-hidden="true" />
                </Link>
              </div>
              <LayerChips nodes={allNodes} selected={selectedLayer} onSelect={setSelectedLayer} />
            </div>
              ...
              <div className="flex items-center border-b">
                {(["sites", "articles"] as const).map((tab) => (
                  <button
                    ...
                    className={`px-4 py-3 text-sm font-medium capitalize border-b-2 transition-colors cursor-pointer ${
                      viewMode === tab
                        ? "border-primary text-foreground"
                        : "border-transparent text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    {tab === "sites" && <Building2 className="h-3.5 w-3.5 inline mr-1.5 -mt-0.5" aria-hidden="true" />}
                    {tab === "articles" && <FileText className="h-3.5 w-3.5 inline mr-1.5 -mt-0.5" aria-hidden="true" />}
                    {tab}
                  </button>
                ))}
                ...
                {viewMode === "sites" && (
                  <div className="divide-y">
                    {filteredSites.map((site) => (
                      ...
                      >
                        <div className="h-9 w-9 rounded-lg bg-secondary flex items-center justify-center text-lg font-bold text-muted-foreground" aria-hidden="true">
                          {site.title[0]}
                        </div>
            ...
            <div className="bg-card rounded-xl border p-4">
              ...
              <div className="space-y-3">
                {recentArticles.slice(0, 4).map((article) => (
                  ...
                  >
                    <span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/40 mt-2 shrink-0" aria-hidden="true" />
                    <div>
